...
Khi cần kiểm soát truy cập chi tiết hơn, tích hợp `OpaAuthorizationService` trước khi proxy tới dịch vụ downstream. Xem thêm [OPA Integration Guide](opa-integration-guide.md) để cấu hình OPA server và kết nối từ .NET.
...
Xem thêm [BFF Guide](security/bff.md) và [OPA Integration Guide](opa-integration-guide.md) để biết cách cấu hình chi tiết.
